...
# dim: delimiter
def PlotRocAndPRCurvesAndMetrics(truth, pred, args_prefix,prf, csvPre=""):
    ...
    """plt.title('ROC curve:{}'.format(args_prefix))
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.plot(fpr, tpr)
    plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='r', label='Chance', alpha=.8)
    # # plt.show()
    plt.savefig("plots/ROC_{}.pdf".format(args_prefix))
    plt.close()"""

    # PR curve
    precision, recall, thresholds = precision_recall_curve(truth, pred)
    #print("DDDDDD"+str(precision)+"DDDDD"+str(recall))
    #print(precision[-10:])
    #print(recall[-10:])
    aupr = auc(recall, precision)
    print("Area under PR curve: ", aupr)
    """plt.title('PR curve:{}'.format(args_prefix))
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.plot(recall, precision)
     # plt.show()
    plt.savefig("plots/PR_{}.pdf".format(args_prefix))# + args.prefix + "/PR_" + csvPre + ".pdf")
    plt.close()"""
    ...
    while True:
        ...
        if path.exists(result_fn):
            if(count_num_of_lines_exclude_end_of_line_in_a_file(result_fn) == len(cur_label) - 8):
                ...
                print(count_num_of_lines_exclude_end_of_line_in_a_file(result_fn), len(cur_label))
...
def main():
    ...
    PlotRocAndPRCurvesAndMetrics(truth=label_np.ravel(), pred=value_np.ravel(), args_prefix=prefix,prf=prefix,csvPre="")
